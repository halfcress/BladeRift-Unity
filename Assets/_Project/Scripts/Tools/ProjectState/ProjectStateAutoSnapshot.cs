...
public static class ProjectStateAutoSnapshot
{
    ...
    private static void TakeAutoSnapshot()
    {
        try
        {
            ...
            ProjectStateChatStateWriter.UpdateAutoBlock(full); // AUTOGENERATED guncelle
            ProjectStateReadmeUpdater.UpdateReadme(full);      // README guncelle
    ...
    private static void CleanupAutoSnapshots(int maxKeep)
    {
        ...
        for (int i = maxKeep; i < autoFiles.Length; i++)
        {
            ...
            File.Move(src, dst);
            Debug.Log($"[AutoSnapshot] Archived: {Path.GetFileName(src)}");
        }
    }
}
